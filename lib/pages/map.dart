...
import 'package:kitit/services/denue_repository.dart';
import 'package:kitit/services/concentration_service.dart';
import 'package:kitit/services/cache_service.dart';
import 'package:kitit/widgets/concentration_panel.dart';
import 'package:kitit/widgets/concentration_legend.dart';
...
class _Map1State extends State<Map1> {
  ...
  // Helper para recolorear polígonos
  Set<Polygon> _recolorPolygons(Set<Polygon> src, Color fill) {
    final out = <Polygon>{};
    for (final p in src) {
      out.add(Polygon(
  ...
  Widget build(BuildContext context) {
    ...
    void onTap(LatLng position) async {
      ...
      final resultados =
          await MySQLConnector.getData(placemarks[0].postalCode);
      print(
      ...
      if (resultados[0].length > 0) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content:
                Text('No se econtraron datos en el lugar seleccionado'),
          ),
        );
      }
    ...
    Set<Polygon> paintPolygons() {
      final base = hammerIsTaped ? _polygonSetDisable : _polygonSet;
      if (showConcentrationLayer && _currentConcentration != null) {
        return _recolorPolygons(base, _currentConcentration!.color);
      }
      return {...base};
    }
                    ...
                    return SpeedDial(
                      ...
                      children: [
                        ...
                        SpeedDialChild(
                          ...
                          onTap: () async {
                            if (postalCode == null ||
                                actividadEconomica.value.isEmpty) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                    content: Text(
                                        'Selecciona zona y actividad económica')),
                              );
                              return;
                            }

                            // Usa el punto seleccionado o el centro por defecto
                            final lat = postionOnTap?.latitude ??
                                _kGooglePlex.target.latitude;
                            final lon = postionOnTap?.longitude ??
                                _kGooglePlex.target.longitude;

                            // Cache
                            final act = actividadEconomica.value;
                            final cp = postalCode;
                            ConcentrationResult? result =
                                CacheService.get(act, cp);
                            ...
                            if (result == null) {
                              ...
                              result = ConcentrationService.compute(
                                entries: entries,
                                activity: act,
                                postalCode: cp,
                              );

                              CacheService.put(act, cp, result);
                            }

                            setState(() {
                              _customInfoWindowController.hideInfoWindow!();
                              _currentConcentration = result;
                            ...
                            await showModalBottomSheet(
                              ...
                              builder: (_) =>
                                  ConcentrationPanel(result: result!),
                            );
                          },
                        ),
                        SpeedDialChild(
        ...
        onTap: () async {
          ...
          setState(
            () {
              ...
              var contador = 0;
              if (_polygonSet.last.polygonId ==
                  const PolygonId("seleccion")) {
                for (var element in _polygonSet) {
